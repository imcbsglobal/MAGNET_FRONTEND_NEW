                              ...
                              <span className="password">
                                ...
                                <span className="password-toggle">
                                  <span className="password-value">
                                    {revealedPasswords[administrator.id] || '••••••••'}
                                  </span>
                                  {loadingPassword[administrator.id] ? (
                                    <span className="password-loading" style={{ fontSize: '0.7rem', color: '#a3aed0' }}>...</span>
                                  ) : revealedPasswords[administrator.id] ? (
                                    <button
                                      type="button"
                                      className="password-eye-btn"
                                      onClick={() => setRevealedPasswords(prev => { const n = {...prev}; delete n[administrator.id]; return n; })}
                                      title="Hide password"
                                    >
                                    ...
                                    <button
                                      ...
                                      className="password-eye-btn"
                                      onClick={async () => {
                                        setLoadingPassword(prev => ({ ...prev, [administrator.id]: true }));
                                        try {
                                          const res = await api.get(`admins/${administrator.id}/password/`);
                                          setRevealedPasswords(prev => ({ ...prev, [administrator.id]: res.data.password }));
                                        } catch {
                                          alert('Failed to reveal password');
                                        } finally {
                                          setLoadingPassword(prev => ({ ...prev, [administrator.id]: false }));
                                        }
                                      }}
                                      title="Show password"
                                    >
